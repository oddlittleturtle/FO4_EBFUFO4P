...
Event Actor.OnPlayerFireWeapon(Actor akSender, Form akBaseObject)
	...
	;UFO4P 2.0.3 Bug #23092: added sanity check:
	if locToTest
...
;    9 - Heal 
	...
	;UFO4P 2.0.0 Bug #20812: Added sanity check for alTarget being none:
	if akTarget && akSender == Companion
...
Function SetCompanion(Actor ActorToMakeCompanion, bool SetCompanion = true, bool FillCompanionAlias = true, bool SuppressDismissMessage = false)
	...
	;/----------------------------------------
	Patch for Everyone's Best Friend and UFO4P
	...
	if CAS == CompanionToDismiss ;is actor we are trying to dismiss actually the current companion?
		...
		if SuppressDismissMessage == false
			...
			if WorkshopHome == None
				...
				;UFO4P 2.0.4 Bug #24382: replaced the previous line with the following code block:
				;If the companion was never sent to any workshop before, there will be no linked ref, so this needs to be checked
				;before trying to retrieve the workshop location.
				ObjectReference CompanionLinkedRef = CompanionToDismiss.GetLinkedRef(workshopItemKeyword)
				if CompanionLinkedRef
					WorkshopHome = (CompanionLinkedRef as WorkshopScript).myLocation
				endif
	...
	if DAS
		...
		if SuppressDismissMessage == false
			...
			if WorkshopHome == None
				...
				;UFO4P 2.1.2 Bug #28259: replaced the previous line with the following code block:
				;If dogmeat was never sent to any workshop before, there will be no linked ref, so this needs to be checked
				;before trying to retrieve the workshop location.
				ObjectReference DASLinkedRef = DAS.GetLinkedRef(workshopItemKeyword)
				if DASLinkedRef
					WorkshopHome = (DASLinkedRef as WorkshopScript).myLocation
				endif
...
Function HandleMessageFromSAEClutterTrigger(SAEClutterTriggerScript triggerbox, bool AddMe)
		...
		;UFO4P 2.0.1 Bug #22189: Added the following lines to replace the vanilla code below:
		;As it turned out, the formlist may eventually contain 'none' entries. Once this occurs, this function and the HandleMessageFromSASTrigger function below
		;will stop working forever, because this script did not expect this to happen and had no code for handling this.
		
		int SA_CurrentSAEClutterTriggersListSize = SA_CurrentSAEClutterTriggersList.getSize()
		SAEClutterTriggerScript FirstValid = none

		if SA_CurrentSAEClutterTriggersListSize > 0
			int i = 0
			while i < SA_CurrentSAEClutterTriggersListSize && FirstValid == none
				FirstValid = SA_CurrentSAEClutterTriggersList.getAt(i) as SAEClutterTriggerScript
				i += 1
			endWhile
		endif

		if FirstValid
			if FirstValid.LocEncKeyword
				SetSAE(FirstValid.LocEncKeyword, shouldSetSAEClutter = true, shouldSetSAECombatant = false, SettingObject = triggerbox)
			else
				traceconditional(self, "SA_CurrentSAEClutterTriggersList FirstValid.LocEncKeyword == None", DebugTrace_SA, 1)
			endif
		else
			traceconditional(self, "SA_CurrentSAEClutterTriggersList is empty", DebugTrace_SA)
			ClearSAEClutter()
		...
		;-------------------------------------------------------------------------------------------------------------------------------------------
		;	UFO4P 2.0.1 Bug #22189: Replaced vanilla code:
		;-------------------------------------------------------------------------------------------------------------------------------------------
...
Function HandleMessageFromSASTrigger(SASTriggerScript triggerbox, bool AddMe)
		...
		;UFO4P 2.0.1 Bug #22189: Added the following lines to replace the vanilla code below:
		;See notes on the HandleMessageFromSAEClutterTrigger function for more information.
		
		int SA_CurrentSASTriggersListSize = SA_CurrentSASTriggersList.getSize()
		SASTriggerScript FirstValid = none

		if SA_CurrentSASTriggersListSize > 0
			int i = 0
			while i < SA_CurrentSASTriggersListSize && FirstValid == none
				FirstValid = SA_CurrentSASTriggersList.getAt(i) as SASTriggerScript
				i += 1
			endWhile
		endif

		if FirstValid
			if FirstValid.LocSetKeyword
				SetSAS(FirstValid.LocSetKeyword, triggerbox)
			else
				traceconditional(self, "SA_CurrentSASTriggersList FirstValid.LocSetKeyword == None", DebugTrace_SA, 1)
			endif
		else
			traceconditional(self, "SA_CurrentSASTriggersList is empty", DebugTrace_SA)
...
Function SetAutonomyPrivate(ScriptObject CallingObject, bool AutonomousActivityAllowed = True)
	...
	;UFO4P 2.0.6 Bug #23394: added this block of code:
	;There's evidence that this array may overflow, presumably due to invalid script objects passed in. Once this happens, the followers' autonomy
	;control gets permanently stuck and the only way to make it work again is to clear the array. An even better solution would be to prevent any
	;invalid objects from being passed in, or to identify them by specific checks before proceeding, but these objects are currently not known.
	;Thus, for the time being, we keep to clearing the array and to log the objects that are in the array when it overflows.
	if AutonomyDisallowedByObjects.length >= 128
		trace (self, "SetAutonomy(): Array overflow. Logging objects and clearing array ...")
		int i = 0
		while i < 128
			trace (self, "  " + i + ": " + AutonomyDisallowedByObjects[i])
			i += 1
		endWhile
